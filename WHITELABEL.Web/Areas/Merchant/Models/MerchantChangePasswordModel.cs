namespace WHITELABEL.Web.Areas.Merchant.Models
{
    ...
    public class MerchantRailRDSPasswordModel
    {
        public long MEM_ID { get; set; }
        [Required]
        [Display(Name = "Old Password")]
        [DataType(DataType.Password)]
        [MaxLength(15, ErrorMessage = "RDS Old Password not greater then 15 digit")]
        [MinLength(5, ErrorMessage = "Old Password not less then 5 digit")]
        public string OldUser_pwd { get; set; }
        [Required]
        [Display(Name = "Password")]
        [DataType(DataType.Password)]
        [MaxLength(12, ErrorMessage = "RDS Password not greater then 12 digit")]
        [MinLength(5, ErrorMessage = "RDS Password not less then 5 digit")]
        public string User_pwd { get; set; }
        [Required]
        [Display(Name = "Confirm Password")]
        [DataType(DataType.Password)]
        [StringLength(12, ErrorMessage = "Comfirm RDS password must be at least 12 characters long", MinimumLength = 5)]
        [System.ComponentModel.DataAnnotations.Compare("User_pwd", ErrorMessage = "The new RDS password and confirm RDS passwords are not matching")]
        public string CONFIRMPASSWORD { get; set; }
    }
