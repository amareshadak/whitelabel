...
namespace WHITELABEL.Web.Areas.Distributor.Controllers
{
    ...
    public class DistributorDashboardController : DistributorBaseController
    {
        ...
        public ActionResult Index()
        {
            if (Session["DistributorUserId"] != null)
            {
                try
                {
                    ...
                    //                                CR_Col = (z.DR_CR=="CR"?"CR":"0"),
                    //                                DR_Col = (z.DR_CR == "DR" ? "DR" : "0"),
                    //                                AMOUNT = z.Amount,
        ...
        public PartialViewResult MemberGrid()
        {
            ...
            {
                var db = new DBContext();
                //DateTime dateva = DateTime.Now.AddDays(-1);
                //DateTime Todatday = Convert.ToDateTime(dateva.ToString("yyyy-MM-dd"));

                //var transactionlistvalue = (from x in db.TBL_ACCOUNTS
                //                            join y in db.TBL_MASTER_MEMBER on x.MEM_ID equals y.MEM_ID
                //                            where x.MEM_ID == 100002 && x.TRANSACTION_DATE == Todatday
                //                            //where searchTest.Contains(x.MEM_ID.ToString()) && x.TRANSACTION_DATE== Todaydate
                //                            select new
                //                            {
                //                                SLN = x.ACC_NO,
                ...
                //return PartialView("MemberGrid", transactionlistvalue);
            }
            catch (Exception ex)
            {
                throw ex;
