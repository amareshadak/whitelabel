...
{
    [Authorize]
    public class DistributorRailIdAllocationController : DistributorBaseController
    {
        ...
        // GET: Distributor/DistributorRailIdAllocation
        ...
        public async Task<JsonResult> GetMemberName(string prefix)
        {
            try
            {
                ...
                var OperatorValue = await (from oper in db.TBL_MASTER_MEMBER
                                           where oper.UName.StartsWith(prefix) && oper.MEMBER_ROLE == 5 && oper.INTRODUCER==MemberCurrentUser.MEM_ID
                                           select new
                                           {
        ...
        public async Task<JsonResult> POSTMerchantSellRailId(TBL_RAIL_ID_ALLOCATION objRail)
        {
            initpage();////
             var db = new DBContext();
                ...
                try
                {
                    ...
                    if (WhitelabelInfo.RAIL_ID_QUANTITY >= RailQty)
                    {
                        ...
                        if (WLP_Info.BALANCE >= objRail.GROSS_AMOUNT)
                        {
                            ...
                            return Json("Rail Id Sell to Merchant", JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json("There is no suffcient Balance in Merchant for selling of rail Id. ", JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
                        return Json("There is no suffcient Rail Id for sell.Please Purchase Rail Id From White Label ", JsonRequestBehavior.AllowGet);
                    }
        ...
        public ActionResult MerchantRailIDAllocationList()
        {
            if (Session["DistributorUserId"] != null)
            {
                initpage();
                var db = new DBContext();
                var DistributorRailIdQty = db.TBL_MASTER_MEMBER.FirstOrDefault(x => x.MEM_ID == MemberCurrentUser.MEM_ID).RAIL_ID_QUANTITY;
                ViewBag.RailIDQty = DistributorRailIdQty;
                return View();
            }
            else
            {
