            ...
            try
            {
                ...
                var DistributorInfo = (from x in db.TBL_MASTER_MEMBER
                                       ...
                                       select new
                                       {
                                           ...
                                           DistUiqueId = db.TBL_MASTER_MEMBER.Where(a => a.MEM_ID == x.INTRODUCER && a.MEMBER_ROLE == 4).Select(q => q.MEM_UNIQUE_ID).FirstOrDefault(),
                                           DistributorId = x.MEM_ID,
                                           UserName = x.UName,
                                           EmailId = x.EMAIL_ID,
                                           MobileNo = x.MEMBER_MOBILE,
                                           JoiningDate = x.JOINING_DATE,
                                           Member_UniqueId=x.MEM_UNIQUE_ID

                                       }).AsEnumerable().Select(z => new TBL_MASTER_MEMBER
                                       {
                                           SuperName = z.Super_Name,
                                           SUPER_ID = z.Super_ID,
                                           DistributorName=z.DistUiqueId,
                                           MEM_ID = z.DistributorId,
                                           UName = z.UserName,
                                           EMAIL_ID = z.EmailId,
                                           MEMBER_MOBILE = z.MobileNo,
                                           JOINING_DATE = z.JoiningDate,
                                           MEM_UNIQUE_ID=z.Member_UniqueId
                                       }).ToList();
